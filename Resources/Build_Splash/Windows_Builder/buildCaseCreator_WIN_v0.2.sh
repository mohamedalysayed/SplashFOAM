...
# Validate and convert PNG icon to ICO
prepare_icon() {
    if [ ! -f "$ICON_PATH" ]; then
        echo -e "${RED}Error: Icon file not found at '$ICON_PATH'.${NC}"
        exit 1
    fi

    ICON_ICO="${ICON_PATH%.*}.ico"
    if [ ! -f "$ICON_ICO" ]; then
        echo -e "${BLUE}Converting PNG icon to ICO format...${NC}"
        python3 - <<EOF
from PIL import Image
Image.open("$ICON_PATH").save("$ICON_ICO", format="ICO")
EOF
        echo -e "${GREEN}Icon converted successfully: $ICON_ICO${NC}"
    else
        echo -e "${GREEN}Icon is ready for use: $ICON_ICO${NC}"
    fi
...
ensure_python_env() {
    ...
    if ! command -v python3 &>/dev/null; then
        echo -e "${RED}Python is not installed. Please install it and try again.${NC}"
        exit 1
    fi
    ...
    if [[ $(echo -e "$PYTHON_VERSION\n$REQUIRED_VERSION" | sort -V | head -n1) != "$REQUIRED_VERSION" ]]; then
        echo -e "${RED}Python version $PYTHON_VERSION is too old. Please upgrade to $REQUIRED_VERSION or higher.${NC}"
        exit 1
    fi

    echo -e "${GREEN}Python 3 is available and compatible.${NC}"
}
...
ensure_dependencies() {
    ...
    REQUIRED_MODULES=(
        ...
        "PySide6"
        "vtk"
        "Pillow"  # For PNG to ICO conversion
    )
...
validate_build() {
    ...
    if [ -f "dist/$OUTPUT_NAME/$OUTPUT_NAME.exe" ]; then
        echo -e "${GREEN}=================================================${NC}"
