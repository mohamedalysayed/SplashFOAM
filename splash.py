...
class ReplacePropertiesPopup:
    def __init__(self, parent, thermo_type_params, mixture_params, old_values_thermo_type, old_values_mixture):
        ...
        ttk.Button(self.popup, text="Update", command=self.replace_mixture_values).grid(row=row_counter, column=2, pady=10, padx=10)
    ...
    def replace_mixture_values(self):
        ...
        for param, entry in self.new_values_thermo_type.items():
            value = entry.get()
            if value != "":
...
\\*---------------------------------------------------------------------------*/\n"""
    ...
    def on_fuel_selected(self, event):
        selected_fuel = self.selected_fuel.get()
        if selected_fuel == "Methanol":
            print("Selected Methanol. Execute command for Methanol.")
        elif selected_fuel == "Ammonia":
            print("Selected Ammonia. Execute command for Ammonia.")
        elif selected_fuel == "Dodecane":
            print("Selected Dodecane. Execute command for Dodecane.")
