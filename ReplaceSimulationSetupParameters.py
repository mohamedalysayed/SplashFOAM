...
class ReplaceSimulationSetupParameters:
    def __init__(self, parent, constant_params, system_params, existing_values):
        ...
        if confirmation:
            ...
            # Create entry fields for each parameter in the "Constant Parameters" group
            self.create_label_and_entry_widgets(constant_frame, self.constant_params)

            # Create entry fields for each parameter in the "System Parameters" group
            self.create_label_and_entry_widgets(system_frame, self.system_params)
    ...
    def create_label_and_entry_widgets(self, frame, params_dict):
        # Create entry fields for each parameter in the frame
        for file_name, param_list in params_dict.items():
            # Create a LabelFrame for each file
